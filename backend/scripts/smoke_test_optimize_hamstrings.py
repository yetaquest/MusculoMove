...
def main() -> None:
    tightness_settings = [
        {
            "muscle_group_id": "hamstrings_lateral",
            "severity": 0.75,
    ...
    result = solve_adapted_pose(
        selected_muscle_ids=["hamstrings_lateral"],
        tightness_settings=tightness_settings,
        maxiter=40,
    )

    print("\n=== OBJECTIVE BREAKDOWN: BASELINE ===")
    print(result["baseline_breakdown"])

    print("\n=== OBJECTIVE BREAKDOWN: ADAPTED ===")
    print(result["adapted_breakdown"])
    ...
    print("\n=== HAMSTRINGS LATERAL BEFORE / AFTER ===")
